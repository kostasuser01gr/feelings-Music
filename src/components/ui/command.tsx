...
const CommandDialog = ({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        showClose={true}
        className="overflow-hidden p-0 gap-0 max-w-2xl [&>button]:z-[100]"
      >
...
>(({ className, ...props }, ref) => (
  <div className="flex items-center border-b border-[var(--border)] px-3" cmdk-input-wrapper="">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        "flex h-12 w-full rounded-md bg-transparent py-3 text-sm outline-none placeholder:text-[var(--muted-foreground)] disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none",
        className
      )}
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.List
    ref={ref}
    className={cn("max-h-[min(60vh,400px)] overflow-y-auto overflow-x-hidden p-2", className)}
    role="listbox"
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default select-none items-center gap-2 rounded-md px-2 py-2.5 text-sm outline-none aria-selected:bg-[var(--muted)] aria-selected:text-[var(--foreground)] data-[disabled]:pointer-events-none data-[disabled]:opacity-50 focus:bg-[var(--muted)] focus:text-[var(--foreground)]",
      className
    )}
    role="option"
    {...props}
